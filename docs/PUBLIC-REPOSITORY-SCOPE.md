...
本项目的本地工作区包含完整开发历史、阶段补丁、浏览器截图、真实 HOST 验收记录和临时运行日志。它们服务于本地审计，不适合进入公开 GitHub 仓库。公开交接快照已发布到 `jzg-lab/Server-and-Project-AI-Hosting` 的 `main`；源码快照基线提交为 `f4a5ae5c48bc5468b0d3676063e1752c7515b2e3`，之后只允许追加文档同步提交。
...
本地完整工作区为了保留审计历史，仍可能跟踪包含真实环境地址的旧测试、脚本或验收材料；公开快照会将这类地址替换为文档保留地址或 fixture 值。不能因为本地文件仍保留历史材料，就推断这些材料已经进入公开仓库；发布前必须对公开快照目录和 Git 跟踪清单单独扫描。
